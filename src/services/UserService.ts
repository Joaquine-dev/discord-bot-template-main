...
export class UserService {
  ...
  static async createUserIfNotExists(client: any, member: GuildMember) {
    ...
    let user = await userRepository.findOne({ where: { discordId: member.id, guildId: member.guild.id } });
    if (!user) {
      user = userRepository.create({ discordId: member.id, username: member.user.username, nickname: member.nickname || member.user.username, guildId: member.guild.id });
  ...
  static async getUserById(client: any, userId: string, guildId: string) {
    ...
    return await userRepository.findOne({ 
      ...
      select: ["id", "discordId", "username", "nickname", "createdAt", "updatedAt"]
     });
  }
  static async updateUser(client: any, userId: string, guildId: string, data: any) {
    const userRepository = client.database.getClient().getRepository(User);
    const user = await userRepository.findOne({ where: { discordId: userId, guildId: guildId } });
    if (user) {
